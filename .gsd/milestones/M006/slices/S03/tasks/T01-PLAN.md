...
# T01: Create ValidationGate + InternalServiceLayer, wire into app.py, write tests

Create two new gateway modules, refactor _mcp_dispatch to delegate to the service layer, and write tests/test_m006_gateway.py proving R006 and R007.
...
- ``src/mcp_agent_factory/gateway/app.py` — existing gateway with inline tool dispatch to refactor`
- ``src/mcp_agent_factory/models.py` — AddInput, EchoInput, AddOutput models`
- ``tests/test_gateway.py` — existing suite that must still pass`

## Expected Output

- ``src/mcp_agent_factory/gateway/validation.py` — ValidationGate class`
- ``src/mcp_agent_factory/gateway/service_layer.py` — InternalServiceLayer class`
- ``src/mcp_agent_factory/gateway/app.py` — _mcp_dispatch refactored to delegate to _service_layer`
- ``tests/test_m006_gateway.py` — 2+ tests proving R006/R007`

## Verification

cd /home/luca/Documents/Misc/mcp-agent-factory/.gsd/worktrees/M006 && MCP_DEV_MODE=1 pytest tests/test_m006_gateway.py tests/test_gateway.py -v
