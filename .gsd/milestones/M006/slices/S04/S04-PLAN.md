...
**Goal:** Add IdempotencyGuard, DistributedLock, OutboxRelay, and CircuitBreaker to the streams package, covering R008–R014 with 7 synchronous tests against fakeredis.
...
## Tasks
- [x] **T01: Implemented IdempotencyGuard, DistributedLock, OutboxRelay, and CircuitBreaker with 7 passing tests covering R008–R014.** — Create `src/mcp_agent_factory/streams/idempotency.py` with three classes and `src/mcp_agent_factory/streams/circuit_breaker.py` with one class, export them from `streams/__init__.py`, and write `tests/test_m006_reliability.py` with 7 tests covering all requirements R008–R014.
...
And extend `__all__`.
  - Estimate: 45m
  - Files: src/mcp_agent_factory/streams/idempotency.py, src/mcp_agent_factory/streams/circuit_breaker.py, src/mcp_agent_factory/streams/__init__.py, tests/test_m006_reliability.py
  - Verify: PYTHONPATH=src pytest tests/test_m006_reliability.py -v
